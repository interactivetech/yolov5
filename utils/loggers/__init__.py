...
def plot_metrics(save_dir,core_context):
...
class Loggers():
    ...
    def on_fit_epoch_end(self, vals, epoch, best_fitness, fi,core_context=None):
        ...
        if core_context is not None:
            core_context.train.report_validation_metrics(
                    steps_completed = int(epoch),metrics={k:v.item() if isinstance(v,torch.Tensor) else v for k,v in x.items()})
        if self.tb:
            for k, v in x.items():
